...
export default function SignUp() {
  ...
  const error={};
  const email_pattern=/^[0-9]+@iiitu\.ac\.in$/;
  const password_pattern=/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*]).{8,}$/;
   ...
   if(email===""){
    ...
    error.email="Please Enter Your Institute Email ID";
   }
   ...
   if(password===""){
    ...
    error.password="Password should have atleast one small letter, one capital letter and one special character";
   }
   seterrors(error);
